    ...
    def __call__(self, group: Group):
        ...
        for i, (dep, idxs) in enumerate(group):
            ...
            if prune_fn in [
                ...
                print("local_imp",local_imp.size())
...
class OBDCImportance(GroupNormImportance):
    ...
    def adjust_fisher(self, group, idxs):
        for i, (dep, id) in enumerate(group):
            layer = dep.target.module
            print("self.Fisher[layer]", self.Fisher[layer].size())
...
class HRankImportance(GroupNormImportance):
    """EigenDamage: Structured Pruning in the Kronecker-Factored Eigenbasis:
       http://proceedings.mlr.press/v97/wang19g/wang19g.pdf
    """
        ...
        for i, (dep, id) in enumerate(group):
            ...
            if layer in self.modules:
                if layer.weight.grad is not None:
                    ...
                    indices_to_keep = list(range(self.Fisher[layer].shape[1]))
                    for idx in idxs:
                        indices_to_keep = [i for i in indices_to_keep if
                                           not (idx * kernel_size <= i < (idx + 1) * kernel_size)]
                    self.Fisher[layer] = torch.index_select(self.Fisher[layer], 1, torch.LongTensor(indices_to_keep).to(
                        self.Fisher[layer].device))
